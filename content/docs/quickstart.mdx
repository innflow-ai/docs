...
<Steps>
  <Step title="Create your account">
    Open [Create account](https://app.innflow.ai/signup). Continue with Google, or enter your name and email and verify the code sent to your inbox.
  </Step>
  ...
  <Step title="Pick a plan">
    Choose the plan that fits your needs. The **Free** plan includes credits for exploring workflows. Check your account for its current allowance, and compare paid options on the [pricing page](https://innflow.ai/pricing).
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Add action nodes">
    ...
    - **Forms** — Manage Typeform or Google Forms and publish Innflow intake forms
    ...
    - **Social publishing** — LinkedIn, Meta, X, Reddit, Bluesky, Threads, Telegram, Pinterest, and WhatsApp
    - **Advanced protocols** — MCP servers and A2A-compatible agents
    - **Logic** — Conditions, routers, approvals, event waits, code execution, HTTP requests, field editing, array transforms, CSV export, tables, and knowledge base search
  </Step>
  <Step title="Connect and configure">
    ...
    Use the variable picker to reference an upstream value, such as `{{tables.rows}}`. Output paths differ by node; do not add an `output` level unless it appears in that node’s actual result.
  </Step>
...
<Columns cols={2}>
  ...
  <Card title="Review Executions" icon="list-checks" href="/features/executions-inbox">
    Inspect outputs, debug failures, and act on Inbox notifications.
  </Card>
</Columns>
