...
Triggers start a workflow run from a specific entry point. New workflows support multiple triggers automatically, including multiple triggers of the same type.

## Adding another trigger

Add the next trigger from the node picker, universal search, or the canvas context menu. You can also duplicate an existing trigger. No independent-trigger setting is required. Saving a second trigger upgrades an existing draft automatically while preserving its existing node IDs.

Undeploy a workflow before changing its entry points, and wait for pending or running executions to finish. Configure and test each entry point before deploying again. Copies are paused and receive new variable names; configure their connections before enabling them.

Each event starts only its selected trigger path. When **Run** has more than one eligible trigger, choose the trigger to test. API and workflow callers must identify the intended trigger if more than one matches. Existing workflow-only Stripe and Google Forms callback URLs may need to be replaced with the node-specific URLs from the trigger settings.
...
This node appears as **Trigger manually** in the node picker.

Manual triggers can have an output variable. Use the variable picker or the active trigger context (`{{$trigger}}`) rather than assuming a manual run has no trigger metadata.
...
Use **Innflow Trigger** when another Innflow workflow should call this workflow. Define the typed input fields the caller must supply, then select this entry point in the calling workflow’s **Innflow** node.

Required inputs are checked before a run starts. Use distinct names for multiple Innflow entry points so the caller can select the intended one. Reference the actual trigger variable shown in the editor, and use `{{$trigger.nodeId}}` to identify the active root.

See the [Innflow Trigger fields](/tools/reference#innflow-trigger) and [Innflow action](/tools/reference#innflow).
...
| **Cron expression** | Custom recurring schedule for advanced timing |
...
Schedules also appear in the [Scheduled Tasks calendar](/features/scheduled-tasks), where you can inspect and manage recurring work across the workspace.
