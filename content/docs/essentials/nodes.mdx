...
A trigger starts a workflow run. Add more triggers directly; saving a second trigger upgrades an existing draft automatically. Each event starts its selected path. See [adding another trigger](/triggers#adding-another-trigger).
...
<Columns cols={3}>
  ...
  <Card title="Innflow" icon="workflow" href="/triggers#innflow-trigger">Called by another workflow</Card>
...
<Columns cols={2}>
  ...
  <Card title="Forms" icon="file-text" href="/tools/forms">
    Typeform, Google Forms, and public Innflow forms
  </Card>
  ...
  <Card title="Social Publishing" icon="send" href="/tools/social-publishing">
    LinkedIn, Meta, X, Reddit, Bluesky, Threads, Telegram, Pinterest, WhatsApp
  </Card>
...
The [complete tool and operation reference](/tools/reference) lists every standard universal-search entry and its configuration fields. Search **API** for HTTP requests and **Wait** for duration or date-based pauses.
...
Nodes that change external systems can create real side effects during testing. Add [Human Approval](/tools/logic-utility#human-approval) before irreversible actions when review is required.

## Control and long-running nodes

| Node | Use for |
| --- | --- |
| **Condition** | Split into true and false paths using a deterministic rule |
| **Router** | Let AI select one of several named branches |
| **Wait** | Pause for a duration or until a specific time |
| **Human Approval** | Pause until a workspace member approves or rejects |
| **Wait for Event** | Pause until an external callback arrives |
| **Edit Fields** | Legacy node retained in existing workflows; use Code or Transform Array for new workflows |
| **Transform Array** | Map, filter, or reduce a list |
