...
A trigger starts a workflow run. Legacy workflows cannot add a second trigger until independent triggers are enabled, and multiple-trigger authoring must also be available in the environment. See [adding another trigger](/triggers#adding-another-trigger) for setup details.
...
| **Edit Fields** | Reshape an object without writing code |
