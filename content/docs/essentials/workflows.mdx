...
1. **Trigger** — The starting point that initiates execution. Legacy workflows require **Triggers → Enable independent triggers** before another entry point can be added. Multiple-trigger authoring must also be enabled in the environment. See [adding another trigger](/triggers#adding-another-trigger).
