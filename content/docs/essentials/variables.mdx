...
Variables let you pass data from one node to another using double-curly-brace template paths. The exact path depends on the node name and its output schema; use the variable picker and inspect a test run instead of guessing an `output` prefix.
